...
export function FPESuccess() {
  const navigate = useNavigate();
  const location = useLocation();
  const { caseNumber, year } = location.state || {};
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-sm bg-white shadow-lg border border-gray-200 relative animate-slideUp">
        <div className="p-8 text-center">
          {/* Medical Chart/Clipboard Illustration */}
          <div className="mb-8 flex justify-center">
            <svg width="200" height="140" viewBox="0 0 200 140" fill="none">
              ...
              <text x="100" y="113" fill="white" fontSize="6" textAnchor="middle" fontWeight="bold">
                FPE {year}
              </text>
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">
            FPE Completed Successfully!
          </h1>

          <p className="text-gray-600 mb-2">
            Assessment has been registered for year{' '}
            <span className="font-semibold text-green-600">{year}</span>
          </p>

          <p className="text-sm text-gray-500 mb-6">
            Case Number: <span className="font-semibold text-gray-900">{caseNumber}</span>
          </p>

          <div className="bg-green-50 border border-green-200 p-3 mb-6 text-left">
            <p className="text-xs text-green-800">
              <span className="font-semibold">Valid for {year}</span> • This FPE assessment is valid for the current year only and must be renewed annually.
            </p>
          </div>
